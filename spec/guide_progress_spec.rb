...
describe 'Classroom::GuideProgress' do
  before do
    Classroom::Collection::CourseStudents.for('foo').insert!({
  ...
  describe '#update!' do
    before do
      Classroom::Collection::GuidesProgress.for('foo').update!(submission)
    end

    let(:guide_progress) { Classroom::Collection::GuidesProgress.for('foo').where('guide.slug' => 'pdep-utn/foo', 'course.slug' => 'example/foo').as_json[:guides_progress] }
    ...
    context 'when new exercise is submitted for existing guide' do
      before do
        Classroom::Collection::GuidesProgress.for('foo').update! submission.merge({'id' => 'abc1235', 'exercise' => {'id' => 25, 'name' => 'Second Steps', 'number' => 8}})
      end
    ...
    context 'when new submission is submitted for existing exercise' do
      before do
        Classroom::Collection::GuidesProgress.for('foo').update! submission.merge({'id' => 'abc1235'})
      end
    ...
    context 'when same submission is sent twice, it is ignored' do
      before do
        Classroom::Collection::GuidesProgress.for('foo').update! submission
      end
    ...
    context 'when exercise has changed, a new submission updates it' do
      ...
      before do
        Classroom::Collection::GuidesProgress.for('foo').update! submission.merge({'id' => 'abcd1235', 'exercise' => {'id' => 10, 'name' => 'New name', 'number' => 3}})
      end
