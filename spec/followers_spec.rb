...
describe Classroom::Collection::Followers do
  ...
  describe 'post /courses/:course/followers' do
    ...
    context 'when authenticated' do
      ...
      it { expect(Classroom::Collection::Followers.for('example', 'bar').find_by(email: 'aguspina87@gmail.com').to_json).to eq({course: 'example/bar', email: 'aguspina87@gmail.com', organization: 'example', uids: ['social|1']}.to_json) }
    end
    ...
    context 'when repeat follower' do
      ...
      it { expect(Classroom::Collection::Followers.for('example', 'bar').find_by(email: 'aguspina87@gmail.com').uids.count).to eq(1) }
    end
    ...
    context 'when not authenticated' do
      ...
      it { expect(Classroom::Collection::Followers.for('example', 'baz').count).to eq 0 }
    end
  ...
  context 'delete /follower' do
    ...
    it { expect(Classroom::Collection::Followers.for('example', 'bar').find_by(email: 'aguspina87@gmail.com').uids).to eq [] }
  end
  ...
  context 'get /follower' do
    ...
    it { expect(last_response.body).to eq({followers: [{course: 'example/bar', email: 'aguspina87@gmail.com', organization: 'example', uids: ['social|1']}]}.to_json) }
  end
